...
class helpdesk_ticket_native extends helpdesk_ticket {
    ...
    function display_ticket($readonly = false) {
        ...
        if (helpdesk_is_capable(HELPDESK_CAP_ANSWER) and !$readonly) {
            $headstr = "<span class=\"head\">$headstr</span>";
        ...
        if ($isanswerer and !$readonly) {
            ...
            $str .="<br /><small><a class=\"helpdesk-action\" href=\"" . $newuserurl->out() . "\">" .
        ...
        if ($this->firstcontact != null and $showfirstcontact != false) {
            ...
            $help = helpdesk_simple_helpbutton(get_string('firstcontact', 'block_helpdesk'), 'firstcontact');
        ...
        $row[] = $this->get_detail();
        $table->data[] = $row;
        $table->rowclass[end(array_keys($table->data))] = 'helpdesk-detail';
        ...
        if ($isanswerer and !$readonly) {
            ...
            $thead = "<span class=\"head\">$thead</span>";
            $thead .= "<br /><a class=\"helpdesk-action\" href=\"$url\">$string</a>";
        }
        ...
        if ($assigned === false) {
            ...
            foreach ($assigned as $user) {
                ...
                if ($isanswerer and !$readonly) {
                    $row[] = "<a class=\"helpdesk-action\" href=\"$removeurl\">" . get_string('remove') . "</a>";
                }
                $table->data[] = $row;
            }
        }
        ...
        if (!$tags == null) {
            foreach ($tags as $tag) {
                ...
                if ($isanswerer and !$readonly) {
                    $remove = "<br />
                               <small>
                                   <a class=\"helpdesk-action\" href=\"$url\">$removestr</a>
                               </small>";
                } else {
                    $remove = '';
                }
        ...
        if (is_array($udata) or is_object($udata)) {
            ...
            foreach ($udata as $update) {
                ...
                if ($update->type !== false and
                    ...
                    $row = array();
                    $str = get_string('update_type', 'block_helpdesk');
                    $row[] = $str;
                    if ($hd->is_update_hidden($update)) {
                        $row[] = get_string('thisupdateishidden', 'block_helpdesk');
                    } else {
                        $row[] = get_string($update->type, 'block_helpdesk');
                    }
                    $table->head = $row;
                }
                ...
                $table->data[] = $row;
                $table->rowclass[end(array_keys($table->data))] =
                        'helpdesk-user status-' . $update->status;
                ...
                $table->data[] = $row;
                $table->rowclass[end(array_keys($table->data))] =
                        'helpdesk-status status-' . $update->status;
                ...
                if ($update->newticketstatus != null) {
                    ...
                    $table->data[] = $row;
                    $table->rowclass[end(array_keys($table->data))] =
                            'helpdesk-newstatus status-' . $update->status;
                }
                ...
                $table->data[] = $row;
                $table->rowclass[end(array_keys($table->data))] =
                        'helpdesk-timecreated status-' . $update->status;
                ...
                $table->data[] = $row;
                $table->rowclass[end(array_keys($table->data))] =
                        'helpdesk-notes status-' . $update->status .
                        ($lastnote === true ? ' lastnote' : '');
                print_table($table);
                echo '<br />';
                $lastnote = false;
            }
     ...
     * @return string
     */
    function get_idstring($with_number = false) {
        if ($with_number === true) {
            return "(HD-" . $this->id . ")";
        } else {
     ...
     * @return string
     */
    function get_summary($with_number = false) {
        if ($with_number === true) {
            return $this->get_idstring(true) . " " . $this->summary;
        } else {
    ...
    function remove_assignment($userid) {
        global $DB;
        $result = $DB->delete_records('helpdesk_ticket_assignments', 'userid', $userid, 'ticketid', $this->id);
        if ($result) {
            $this->store();
            $urecord = $DB->get_record('user', array('id' => $userid));
            $dat = new stdClass;
    ...
    function fetch($permissionhalt = true) {
        ...
        $updates = $DB->get_records('helpdesk_ticket_update', array('ticketid' => $this->id), 'timecreated DESC');
        $tags = $DB->get_records('helpdesk_ticket_tag', array('ticketid' => $this->id), 'name ASC');
    ...
    function remove_tag($id) {
        ...
        $result = $DB->delete_records('helpdesk_ticket_tag', 'id', $id);
        if (!$result) {
            return false;
        }
        // Lets make an update!
...
function send_admin_emails($data, $id, $real_id) {
    ...
    $bodyhtml = '';
    //$bodyhtml .= get_string('newticket_created', 'block_helpdesk') . '<br/>';
    ...
    $body = '';
    //$body .= get_string('newticket_created', 'block_helpdesk') . ' - ';
