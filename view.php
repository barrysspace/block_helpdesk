...
//echo "DEBUG: $heading";
//echo "DEBUG: "; print_r($USER);

$title = get_string('helpdeskticketviewer', 'block_helpdesk');


$PAGE->set_url('/blocks/helpdesk/view.php');
...
if (!empty($id)) {
    ...
    if (!$ticket) {
        print_error(get_string('ticketiddoesnotexist','block_helpdesk'));
    }
    ...
    echo "<div class=\"clearfix\"></div><div class=\"center2div\">";
    ...
    if (empty($tickets) or !is_array($tickets)) {
        ...
        foreach($tickets as $ticket) {
            ...
            $row = array();
            $row[] = "<a href=\"$ticketurl\">" . $ticket->get_summary(true) . '</a>';
